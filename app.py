...
BREAK_DURATION = 4 * 60 + 59  # 4 min 59 s
...
game_state = {
    ...
    "venue":        "",
    "match_number": 1,
    # NEW: display title (shown big on display screen)
    "display_title": "",
    # NEW: poll state
    "poll": {
        "active": False,
...
def state():
    ...
    return jsonify({
        ...
        "display_title": game_state["display_title"],
        "poll":          game_state["poll"],
        "spotlight":     game_state["spotlight"],
        "show_score_table": game_state.get("show_score_table", False),
        "score_table_rows": game_state.get("score_table_rows", []),
...
# ── Manual chasing/defending ───────────────────────────────────────────────
@app.route("/set_active_team", methods=["POST"])
def set_active_team():
    d = request.get_json(silent=True) or {}
    t = d.get("team")
    if t not in (0, 1):
        return jsonify({"error": "invalid team"}), 400
    game_state["active_team"] = t
    return jsonify({"ok": True})
...
def set_timer():
    ...
    val = max(0, minutes * 60 + seconds)
    game_state["paused_time"] = val
    if game_state["running"]:
        dur = _phase_duration()
        game_state["start_time"] = time.monotonic() - (dur - val)
    return jsonify({"ok": True, "timer": val})
...
# ── Display title ──────────────────────────────────────────────────────────
@app.route("/set_display_title", methods=["POST"])
def set_display_title():
    d = request.get_json(silent=True) or {}
    game_state["display_title"] = str(d.get("title", "")).strip()
    return jsonify({"ok": True})

# ── Poll ───────────────────────────────────────────────────────────────────
@app.route("/set_poll", methods=["POST"])
def set_poll():
    d = request.get_json(silent=True) or {}
...
@app.route("/hide_poll", methods=["POST"])
def hide_poll():
    game_state["poll"]["active"] = False
    return jsonify({"ok": True})
...
def poll_vote():
    d = request.get_json(silent=True) or {}
    idx = d.get("option")
    if isinstance(idx, int) and 0 <= idx < len(game_state["poll"]["votes"]):
        game_state["poll"]["votes"][idx] += 1
    return jsonify({"ok": True, "votes": game_state["poll"]["votes"]})

# ── Player spotlight ───────────────────────────────────────────────────────
@app.route("/set_spotlight", methods=["POST"])
def set_spotlight():
    d = request.get_json(silent=True) or {}
    game_state["spotlight"] = {
        "active":    bool(d.get("active", True)),
...
@app.route("/hide_spotlight", methods=["POST"])
def hide_spotlight():
    game_state["spotlight"]["active"] = False
    return jsonify({"ok": True})

@app.route("/upload_player_photo", methods=["POST"])
def upload_player_photo():
    ...
    fn   = name.strip().lower().replace(" ", "_") + "_" + str(int(time.time())) + ".png"
    path = os.path.join(PLAYER_FOLDER, fn)
    with open(path, "wb") as f:
        f.write(data)
    return jsonify({"ok": True, "path": f"/static/players/{fn}"})

# ── Score table overlay ────────────────────────────────────────────────────
@app.route("/set_score_table", methods=["POST"])
def set_score_table():
    d = request.get_json(silent=True) or {}
    game_state["show_score_table"] = bool(d.get("show", False))
    rows = d.get("rows", [])
    game_state["score_table_rows"] = [
